...
def split_dataset(config: DictConfig, transcripts_path: str, vocab: Vocabulary):
    ...
    trainset_list = list()
    print(config.train.dataset,'training')
    with open('/content/drive/MyDrive/proto/kospeech/data/transcripts.txt','r') as myFile:
       d_len = len(myFile.readlines())
    if config.train.dataset == 'kspon':
        train_num = round(d_len*0.8)
        valid_num = round(d_len*0.2)
